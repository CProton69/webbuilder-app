                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onValueChange={(value: any) => setNewMenu({ ...newMenu, location: value })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              {menus.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-xs">Click "New Menu" to create your first menu</p>
                </div>
              )}
            </div>
          </ScrollArea>
...
function MenuCard({ menu, pages, anchors, onEdit, onDuplicate, onDelete, onUpdateMenu }: MenuCardProps) {
  const itemCount = menu.items.length + menu.items.reduce((sum, item) => sum + item.children.length, 0)
...
function MenuEditor({ menu, pages, anchors, onSave, onCancel }: MenuEditorProps) {
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value: any) => setEditingMenu({ ...editingMenu, location: value })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="border rounded-lg p-4 bg-gray-50 min-h-[200px]">
          {editingMenu.items.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              ...
              <p className="text-xs">Click "Add Item" to create your first menu item</p>
            </div>
          ) : (
            <div className="space-y-2">
...
function MenuItemEditor({ 
  ...
  const getTypeIcon = (type: MenuItem['type']) => {
    switch (type) {
      ...
      case 'custom': return <Link2 className="w-3 h-3" />
      ...
      <div className="ml-6 p-3 bg-gray-50 border rounded space-y-3">
        ...
        <div>
          <Label className="text-xs">
            {item.type === 'page' ? 'Select Page' : 
             item.type === 'anchor' ? 'Select Anchor' : 
             item.type === 'external' ? 'External URL' : 'Custom URL'}
          </Label>
          {item.type === 'page' && (
            <Select
              value={item.url}
