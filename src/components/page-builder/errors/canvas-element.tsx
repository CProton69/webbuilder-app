...
import { Trash2, Copy, Settings, GripVertical, MoreVertical, LayoutTemplate, Columns, Type, FileText, MousePointer, ImageIcon, Box, ChevronUp, ChevronDown, Code, Anchor, Divide, Minimize2, AlertTriangle, Tag, Menu, ChevronRight, FolderOpen, ChevronDown as ChevronDown2, BarChart2, GalleryHorizontalEnd, GalleryVerticalEnd, MapPin, Music, MessageCircle, Film, PanelTop, PanelBottom, Star, Plus } from 'lucide-react'
...
interface CanvasElementProps {
  ...
  onDrop: (newElement: any, targetElement: any, position: 'before' | 'after' | 'inside') => void
...
export function CanvasElement({ 
  ...
  const [position, setPosition] = useState<'before' | 'after' | 'inside'>('after')
  ...
  useEffect(() => {
    if (elementRef.current && dragItem) {
      ...
      if (element.type === 'section' || element.type === 'column' || element.type === 'container') {
        zones.push({
    ...
    return () => {
      ...
      if (elementRef.current) {
        setDropZones(prev => prev.filter(zone => 
          !zone.id.startsWith(`${element.id}-`)
        ))
      }
    }
  ...
  const handleDragStart = (e: React.DragEvent) => {
    ...
    dragImage.className = 'px-3 py-2 bg-blue-600 text-white text-xs rounded-md shadow-lg whitespace-nowrap'
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    if (dragItem && elementRef.current) {
      ...
      if (y < height * 0.25) {
        ...
        newPosition = 'after'
      } else if (element.type === 'section' || element.type === 'column' || element.type === 'container') {
        newPosition = 'inside'
      }
  ...
  const renderElementContent = () => {
    ...
    switch (element.type) {
          ...
          >
            ...
            <h2 
              className={`text-2xl font-bold ${
                ...
                element.content?.alignment === 'right' ? 'text-right' :
                'text-left'
              }`} 
        ...
        return (
          ...
          >
            ...
            <p 
              ...
              contentEditable={isSelected}
            >
              {element.content?.text || 'This is a text paragraph. Click to edit the content.'}
            </p>
          </div>
        )
              ...
              {element.content?.url ? (
                <div className="relative inline-block">
                  <img 
                    ...
                    style={{
                      width: element.content?.width ? `${element.content.width}px` : 'auto',
                      height: element.content?.height ? `${element.content.height}px` : 'auto',
                      objectFit: 'contain'
                    }}
                    onError={(e) => {
                      console.error('Image failed to load:', element.content.url)
                      e.currentTarget.style.display = 'none'
                      const parent = e.currentTarget.parentElement
                      if (parent) {
                        const errorDiv = document.createElement('div')
                        errorDiv.className = 'w-full h-32 bg-red-50 rounded flex items-center justify-center border-2 border-dashed border-red-300'
                        errorDiv.innerHTML = `
                    ...
                    onLoad={() => {
                      console.log('Image loaded successfully:', element.content.url)
                    }}
                  />
                ...
                <div className="w-full h-32 bg-gray-200 rounded flex items-center justify-center border-2 border-dashed border-gray-300">
                  <div className="text-center">
                    ...
                    <div className="text-xs text-gray-400 mt-1">Click "Upload" to add an image</div>
                  </div>
                </div>
              )}
            </div>
          ...
          >
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="text-sm">
                  {element.content?.anchorId ? (
                    <span className="font-mono text-blue-700">#{element.content.anchorId}</span>
                  ) : (
            ...
            {element.content?.anchorId && (
              <div 
                ...
                aria-label={`Anchor point: ${element.content?.label || element.content.anchorId}`}
              />
            )}
          </div>
        )
        ...
        return (
          ...
          >
            ...
            <div className={`p-4 rounded-lg border ${
              element.content?.type === 'success' ? 'bg-green-50 border-green-200 text-green-800' :
              element.content?.type === 'warning' ? 'bg-yellow-50 border-yellow-200 text-yellow-800' :
              element.content?.type === 'error' ? 'bg-red-50 border-red-200 text-red-800' :
      ...
      case 'container':
        ...
        const padding = containerContent.padding || { top: 20, right: 20, bottom: 20, left: 20 }
        const margin = containerContent.margin || { top: 0, right: 0, bottom: 0, left: 0 }
        const background = containerContent.background || { type: 'color', color: '#ffffff' }
        const border = containerContent.border || { style: 'none', color: '#e5e7eb', width: 1, radius: 0 }
        ...
        if (background.type === 'image' && background.image?.url) {
          ...
          containerStyles.backgroundRepeat = background.image.repeat || 'no-repeat'
        }
        ...
        if (background.type === 'gradient' && background.gradient?.colors) {
          ...
          containerStyles.background = `${background.gradient.type}-gradient(${background.gradient.direction || '180deg'}, ${colors})`
        }
        
        // Add flex layout styles
        if (layout === 'flex' && containerContent.flex) {
          containerStyles.display = 'flex'
          containerStyles.flexDirection = containerContent.flex.direction || 'row'
          containerStyles.flexWrap = containerContent.flex.wrap || 'nowrap'
          containerStyles.justifyContent = containerContent.flex.justify || 'flex-start'
        ...
        if (layout === 'grid' && containerContent.grid) {
          ...
          containerStyles.gridTemplateColumns = `repeat(${containerContent.grid.columns || 1}, 1fr)`
          ...
          >
            ...
            <div className="bg-white rounded border p-4">
              <div className={`flex items-center justify-between ${
                ...
                element.content?.layout === 'vertical' ? 'flex-col items-start space-y-4' : ''
              }`}>
          ...
          >
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-600">
                {element.content?.description || 'Enter your description here...'}
              </p>
            </div>
          </div>
        )
          ...
          >
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="text-gray-600 italic mb-3">
                "{element.content?.quote || 'This is a testimonial quote from one of our satisfied customers.'}"
              </div>
              <div className="flex items-center gap-3">
                ...
                {element.content?.tabs?.map((tab: any, index: number) => (
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium ${
                      index === 0 ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-600 hover:text-gray-800'
                    }`}
                  >
              ...
              <div className="aspect-video bg-gray-100 rounded-lg flex items-center justify-center">
                {element.content?.slides?.[0]?.image ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-sm text-gray-500">Add slides to carousel</p>
                  </div>
                )}
              </div>
      ...
      case 'maps':
        return (
          ...
          >
            ...
            <div className="bg-gray-100 rounded-lg flex items-center justify-center" style={{ height: `${element.content?.height || 400}px` }}>
              <div className="text-center">
                <MapPin className="w-12 h-12 text-gray-400 mx-auto mb-2" />
                <p className="text-sm text-gray-600 mb-1">
                  {element.content?.location || 'New York, NY'}
                </p>
                <p className="text-xs text-gray-500">Google Maps will be displayed here</p>
              </div>
            </div>
          </div>
        )
          ...
          >
            ...
            <div className="bg-orange-100 rounded-lg p-4 flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="font-medium text-orange-800 mb-1">SoundCloud Player</div>
